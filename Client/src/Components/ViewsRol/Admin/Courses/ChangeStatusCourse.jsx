...
export const ChangeStatusCourse = ({
 isOpen,
 ...
 const handleChangeStatus = () => {
  ...
  idSelectedCourses.forEach((id) => {
   const updatedCourse = allCourses.find((course) => course.id === id);
   dispatch(changeStatusCourse({ id, status: !updatedCourse.status }));
  });
 };

 useEffect(() => {
  if (success) {
   setChangeStatusCourse(false);
  }
 }, [success]);
  ...
  >
   {idSelectedCourses.length === 0 ? (
    <div className="p-4 bg-gray-200 rounded-xl shadow-md overflow-auto dark:bg-gray-800 dark:text-white flex flex-col justify-center items-center">
     <span>
      DEBE SELECCIONAR EL/LOS CURSO/S QUE DESEA HABILITAR/INHABILITAR
     </span>
     <button
      className="mt-3 rounded-md bg-custom-blue hover:bg-custom-blueOscuro transition duration-200 text-white font-bold py-1 px-2 w-10 h-9"
      onClick={() => setChangeStatusCourse(false)}
     >
      OK
     </button>
    </div>
    ...
    <div className="p-6 bg-gray-200 rounded-xl shadow-md overflow-auto dark:bg-gray-800 dark:text-white">
     <div className="flex flex-col justify-center items-center">
      <span>
       ¿ESTAS SEGURO QUE DESEA HABILITAR/INHABILITAR LOS SIGUIENTES CURSOS?
      </span>
